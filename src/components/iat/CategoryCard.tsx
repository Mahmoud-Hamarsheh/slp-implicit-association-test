...
import { positiveImages, negativeImages } from "./utils/instructionsContent";
...
export const CategoryCard: React.FC<CategoryCardProps> = ({
  ...
  return (
    <div className="text-center p-4 bg-white rounded-lg shadow-sm">
      ...
      {/* Show positive images when needed */}
      {showPositiveImages && categoryLabel.includes("إيجابي") && (
        <InstructionImages images={positiveImages} alt="إيجابي" />
      )}
      
      {/* Show negative images when needed */}
      {showNegativeImages && categoryLabel.includes("سلبي") && (
        <InstructionImages images={negativeImages} alt="سلبي" />
      )}
